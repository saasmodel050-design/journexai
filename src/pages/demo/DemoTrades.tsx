...
import { Search, LineChart, Lock } from 'lucide-react';

const DemoTrades = ({ openModal }: { openModal: (msg?: string) => void }) => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">{trades.length} demo trades</p>
        </div>
        <Button onClick={() => openModal('Create your account to add and save trades.')}>
          + Add Trade
        </Button>
      </div>
      ...
      <div className="glass-card p-4 flex flex-wrap gap-3 items-center">
        ...
        <Select value={filterResult} onValueChange={setFilterResult}>
          <SelectTrigger className="w-[130px] bg-secondary/50 border-border"><SelectValue placeholder="Result" /></SelectTrigger>
          <SelectContent>
        ...
        {strategies.length > 0 && (
          <Select value={filterStrategy} onValueChange={setFilterStrategy}>
            <SelectTrigger className="w-[150px] bg-secondary/50 border-border"><SelectValue placeholder="Strategy" /></SelectTrigger>
            <SelectContent>
            ...
            <TableBody>
              {filtered.map((trade) => (
                <TableRow key={trade.id} className="border-border hover:bg-secondary/30 transition-colors">
                  ...
                  <TableCell className={`font-mono text-sm font-medium ${(trade.pnl || 0) >= 0 ? 'text-neon-green' : 'text-neon-red'}`}>
                  ...
                  <TableCell>
                    <Button
                      ...
                      onClick={() => openModal('Create your account to edit and manage trades.')}
                    >
